...
public class GameScreen implements Screen {
	...
	public GameScreen(final MainGame game) {
		...
		try {
			...
			coordinateSize = IOUtils.getFloatAttributeFromConfigFile(Paths.get(SCALE_CONFIG_PATH), "BASE_TILE_UNIT") * unitScale;
	...
	/**
	 * Method to load assets using the asset manager and assign them to the properties of the class
	 */
	private void initGameObjects() {
		...
		assetManager.load("assets/elf_f_hit_anim_f0.png", Texture.class);
		assetManager.finishLoading();
		room = new TiledRoom(assetManager.get("assets/TestMap.tmx"));
		Texture texture = TextureSizeManager.resizePlayer(assetManager.get("assets/elf_f_hit_anim_f0.png"));
		player = new TiledPlayer(texture);
		placeEntities();
	}
	...
	public void render(float delta) {
		...
		mapRenderer.render();
		game.getBatch().begin();
		game.getBatch().draw(player.getTexture(), player.getXPositionEntityArea(), player.getYPositionEntityArea());
		game.getBatch().end();
